...
2. **Pipeline resume** — **Phase 2.0으로 이동** (정책 미결정: resume vs 취소+재등록. SQLite 도입과 함께 결정)
3. **Usage check** — **완료** (usage_checker.py, 3계층 threshold, TM/WFC 연동)
4. **재알림** — **완료** (wait_for_human_response에 re_notification_interval_hours 추가)
5. **테스트 스위트** — **완료** (85개: Unit/Integration/E2E, run_test.sh)
