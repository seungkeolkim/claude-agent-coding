...
```
 ...
 7. WFC → PR 생성
    ...
    → [auto_merge=false] task status → waiting_for_human_pr_approve
```
...
| | `scripts/hub_api/protocol.py` | ACTION_REGISTRY 확장 (14 → 19개: create_project, resubmit, get_plan, close_project, reopen_project) |
...
| | `scripts/hub_api/protocol.py` | close_project, reopen_project action 등록 (19개) |
...
| **테스트** | `tests/` (226개) | Unit/Integration/E2E + Web DB/Syncer + Lifecycle 테스트 |
...
| auto_merge=true/false | task 00005 (자동 머지), task 00006 (waiting_for_human_pr_approve) |
