...
**status 값:** `submitted` → `planned` → `waiting_for_human` → `in_progress` → `pending_review` → `completed` / `needs_replan` / `escalated` / `failed` / `cancelled`
...
| | HubAPI: `pending()` 확장 | pending_review 상태도 pending 결과에 포함 |
...
| 범위 | 내용 | 예정 Phase |
|------|------|-----------|
| **Task lifecycle 정립** | status 전이 규칙 정비, 비정상 전이 방지, pending_review 처리 흐름 | 2.0 (잔여) |
