...
| 범위 | 내용 | 예정 Phase |
|------|------|-----------|
| **알림** | 완료/실패/PR생성 시 사용자 알림 (cli 능동 알림 → slack/telegram) | 1.4 |
| **Pipeline resume** | 실패 지점부터 자동 재개 (TM 연동) | 1.4 |
| **Usage check** | claude 세션 사용량 threshold 기반 실행 제어 (실제 체크 로직) | 1.4 |
| **재알림 (re-notification)** | waiting_for_human 상태에서 특정 시간 후 응답 없으면 재알림 | 1.4 |
