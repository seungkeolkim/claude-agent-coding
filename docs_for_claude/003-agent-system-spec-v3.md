# Agent System Architecture Specification v3

> Claude Code CLI 기반 24시간 자동 개발 시스템
> 최종 정리: 2026-04-03
> 이전 버전: `docs_history/003-agent-system-spec-v2.md` (v2)
...
| 문서 | 역할 | 참고 수준 |
|------|------|-----------|
| **`003-agent-system-spec-v3.md` (이 문서)** | **현행 설계 명세. 모든 구현은 이 문서를 기준으로 합니다.** | **기준 문서** |
| `005-design-history-archive.md` | v1→v2 변경 배경 + Phase 1.0 핸드오프 병합 아카이브 | 설계 배경이 궁금할 때 |
| `docs_history/003-agent-system-spec-v2.md` | v2 설계 (아카이브) | 참고 불필요 |
...
│
├── docs_for_claude/
│   ├── 003-agent-system-spec-v3.md     # v3 설계 (현행 기준 문서) ★
...
| | `scripts/cli.py` | CLI 프론트엔드 (10개 서브커맨드) |
| | `run_agent.sh` | CLI 진입점 확장 (task 관리 명령 10개 추가) |
| | WFC: `resolve_effective_config()` | 4계층 설정 deep merge |
| | WFC: `request_human_review()` | Planner/Replan 후 human interaction 기록 |
| | WFC: `wait_for_human_response()` | 폴링 기반 승인 대기 (approve/modify/cancel/timeout) |
...
| 범위 | 내용 | 예정 Phase |
|------|------|-----------|
| ~~**알림**~~ | ~~완료/실패/PR생성 시 사용자 알림 (cli 능동 알림 → slack/telegram)~~ | ~~1.4~~ **완료** |
| **Pipeline resume** | 실패 지점부터 자동 재개 (TM 연동). 정책 미결정: resume vs 취소+재등록. 정책 확정 후 구현 | 보류 |
| **Usage check** | claude 세션 사용량 threshold 기반 실행 제어 (실제 체크 로직) | 1.4 |
| ~~**재알림 (re-notification)**~~ | ~~waiting_for_human 상태에서 특정 시간 후 응답 없으면 재알림~~ | ~~1.4~~ **완료** |
| **E2E 테스트장비 연동** | e2e_watcher.sh, 크로스 머신 handoff, SSH 복구 | 1.3 |
| **로컬 E2E 테스트** | 브라우저 없는 API E2E를 로컬 실행. mode: local/remote 옵션 | 1.3 |
| **Chatbot 레이어** | TM 앞단 대화형 인터페이스. 자연어→구조화 명령 변환 | 1.5 |
| **Chatbot 세션 관리** | ~20턴 compress, 프로젝트 전환 시 context reload | 1.5 |
| **메신저 연동** | Slack/Telegram 메시지 수신 → task 생성. Chatbot 경유 | 1.5 |
| **프로토콜 body 정의** | Request/Response envelope, 에러 형식, 첨부파일 base64 규격 | 1.5 |
| **user_preferences slot** | project_state.json에 사용자 선호 저장. 기존 4계층 내 처리 | 1.5 |
| **SQLite 전환 (선택적)** | 대화 이력, 알림 이력, task 조회 캐시. task JSON은 source of truth 유지 | 필요 시 |
| **웹 대시보드** | Task 목록/상세/Timeline, 프로젝트별 필터링 | 2.0 |
...
| **1.3** | E2E 테스트장비 연동, 로컬 E2E | 미착수 |
| **1.4** | 운영 안정화: 알림, Pipeline resume, Usage check, 재알림 | 미착수 |
| **1.5** | Chatbot + 메신저: Chatbot 레이어, 세션 관리, 메신저 연동, 프로토콜 body, user_preferences | 미착수 |
| **2.0** | 웹 대시보드 + 고급 기능: 웹 UI, Merge conflict, task 순서 변경, 강제 실행 | 미착수 |

참고: Phase 1.1(파이프라인 자동화)과 1.2(Planner+Subtask Loop)는 Phase 1.0에서 WFC로 통합 구현됨.
