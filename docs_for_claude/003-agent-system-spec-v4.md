...
| **테스트** | `tests/` (85개) | Unit/Integration/E2E 테스트 스위트 |
...
| **E2E 테스트장비 연동** | e2e_watcher.sh, 크로스 머신 handoff, SSH 복구 | 1.6 |
| **로컬 E2E 테스트** | 브라우저 없는 API E2E를 로컬 실행. mode: local/remote 옵션 | 1.6 |
...
| **1.5** | Chatbot + 메신저: Chatbot 레이어, 세션 관리, 메신저 연동, 프로토콜 body, user_preferences | **다음** |
| **1.6** | E2E 테스트장비 연동, 로컬 E2E | 미착수 |
| **2.0** | 웹 대시보드 + 고급 기능: Pipeline resume, SQLite, 웹 UI, Merge conflict, task 순서 변경 | 미착수 |

참고: Phase 1.1(파이프라인 자동화)과 1.2(Planner+Subtask Loop)는 Phase 1.0에서 WFC로 통합 구현됨. E2E 테스트장비 연동은 1.3→1.6으로 재배치.
