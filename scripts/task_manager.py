...
종료 시그널:
    SIGTERM  — 새 task spawn 중단, 실행 중 WFC에 SIGTERM 전송 후 30초 대기, 타임아웃 시 SIGKILL
...
class TaskManager:
    ...
    def spawn_workflow_controller(self, project_name, task_id, resume=False):
        """
        ...
        Args:
            project_name: 프로젝트명
            task_id: task ID
            resume: True면 --resume 플래그 추가 (중단된 대기 상태에서 재개)
        ...
        mode_str = " (resume)" if resume else ""
        log_info(f"[{project_name}] WFC spawn{mode_str}: task {task_id}")
    ...
    def check_workflow_controller(self, project_name):
        ...
        대기 상태 보존: WFC가 SIGTERM으로 중단되어 task가 아직
        waiting_for_human_plan_confirm/needs_replan 상태이면,
        project status를 idle로 바꾸지 않고 대기 상태를 유지한다.
        """
        ...
        # 대기 상태 보존: task가 아직 human 응답 대기 중이면 idle로 바꾸지 않음
        task_status = self._get_task_status(project_name, task_id)
        if task_status in ("waiting_for_human_plan_confirm", "needs_replan"):
            log_info(
                f"[{project_name}] WFC 중단됨 "
                f"(task {task_id} 대기 상태 유지: {task_status})"
            )
            # current_task_id 유지 (resume 감지용)
            state["current_task_id"] = task_id
            self.update_project_state(
                project_name, task_status, current_task_id=task_id
            )
        else:
            state["current_task_id"] = None
            last_error = task_id if returncode != 0 else None
            self.update_project_state(
                project_name, "idle", current_task_id=None, last_error=last_error
            )
    ...
    def _find_task_file(self, tasks_dir, task_id):
        """
        tasks 디렉토리에서 task_id에 해당하는 JSON 파일을 찾는다.

        파일명 패턴: {task_id}.json 또는 {task_id}-{제목}.json
        """
        # 정확한 이름 먼저
        exact_path = os.path.join(tasks_dir, f"{task_id}.json")
    ...
    def _get_task_status(self, project_name, task_id):
        """
        task JSON 파일에서 현재 status를 읽어 반환한다.

        task_id가 None이거나 파일을 읽을 수 없으면 None 반환.
        """
        if not task_id:
            return None
        tasks_dir = os.path.join(self._projects_dir, project_name, "tasks")
    ...
    # ═══════════════════════════════════════════════════════════
    # Resume — 대기 중 task 재시작
    # ═══════════════════════════════════════════════════════════

    def resume_waiting_tasks(self):
        """
        시작 시 대기 중인 task를 찾아 WFC를 --resume으로 재시작한다.

        TM 재시작 후 1회 호출. project_state.json과 task JSON을 이중 검증하여
        실제로 대기 상태인 task만 resume한다.
        """
        resumable_statuses = {"waiting_for_human_plan_confirm", "needs_replan"}

        for project_name in list(self._project_states.keys()):
            # 이미 WFC 실행 중이면 건너뜀
            if self._project_states[project_name].get("process"):
                continue

            # project_state.json 읽기
            state_path = os.path.join(
                self._projects_dir, project_name, "project_state.json"
            )
            ...
            if status not in resumable_statuses or not task_id:
                continue
            ...
            if task_status not in resumable_statuses:
                log_warn(
                    f"[{project_name}] project_state는 {status}이지만 "
                    f"task {task_id}는 {task_status} — resume 건너뜀"
                )
                continue

            # 기존 WFC 프로세��가 살아있는지 확인 (stale wfc_pid 방어)
            wfc_pid = state.get("wfc_pid")
            if wfc_pid and self._is_process_alive(wfc_pid):
                log_warn(
                    f"[{project_name}] WFC가 이미 실행 중 "
                    f"(PID {wfc_pid}) — resume 건너뜀"
                )
                continue

            log_info(
                f"[{project_name}] 대기 중인 task {task_id} 발견 — "
                f"WFC 재시작 (--resume)"
            )
            self.spawn_workflow_controller(project_name, task_id, resume=True)

    def _check_waiting_task_response(self, project_name):
        """
        WFC가 ��는 상태에서 대기 중인 task에 사용자 응답이 도착했는지 확인한다.

        응답이 도착했으면 WFC를 --resume으로 spawn하고 True를 반환한다.
        auto_approve_timeout도 여기서 확인하여 타임아웃 시 자동 spawn.

        Returns:
            True면 WFC를 spawn했으므로 다른 task 탐색 불필요
        """
        state = self._project_states.get(project_name, {})
        ...
        # task status 확인
        task_status = self._get_task_status(project_name, task_id)
        if task_status not in ("waiting_for_human_plan_confirm", "needs_replan"):
            return False

        # task JSON에서 human_interaction 응답 확인
        tasks_dir = os.path.join(self._projects_dir, project_name, "tasks")
        task_file = self._find_task_file(tasks_dir, task_id)
        if not task_file:
            return False
        ...
        if response:
            # 사용자 응답 도착 → WFC resume
            action = response.get("action", "approve")
            log_info(
                f"[{project_name}] task {task_id} 응답 감지: "
                f"action={action} — WFC 재시작"
            )
            self.spawn_workflow_controller(project_name, task_id, resume=True)
            return True

        # auto_approve_timeout 확인
        requested_at = hi.get("requested_at", "") if hi else ""
        if requested_at:
            try:
                ...
                elapsed_hours = (
                    datetime.now(timezone.utc) - req_time
                ).total_seconds() / 3600

                # effective config에서 timeout 읽기
                state_path = os.path.join(
                    self._projects_dir, project_name, "project_state.json"
                )
                timeout_hours = 24  # 기본값
                if os.path.isfile(state_path):
                    try:
                        ...
                        timeout_hours = (
                            overrides.get("human_review_policy", {})
                            .get("auto_approve_timeout_hours", timeout_hours)
                        )
                    except (json.JSONDecodeError, OSError):
                        pass
                ...
                if elapsed_hours >= timeout_hours:
                    log_warn(
                        f"[{project_name}] task {task_id} auto_approve timeout "
                        f"({timeout_hours}h) 초과 — WFC 재시작"
                    )
                    self.spawn_workflow_controller(
                        project_name, task_id, resume=True
                    )
                    return True
    ...
    def shutdown(self, force=False):
        ...
        Args:
            force: True면 WFC에 SIGKILL 전송, False면 SIGTERM + 타임아웃 대기
        """
        if force:
            ...
            # graceful: WFC에 SIGTERM 전송 → 타임아웃 대기 → 필요 시 SIGKILL
            running_projects = [
                name for name, state in self._project_states.items()
                if state.get("process") and state["process"].poll() is None
            ]
            if running_projects:
                # 1단계: 모든 실행 중 WFC에 SIGTERM 전송
                for project_name in running_projects:
                    process = self._project_states[project_name]["process"]
                    try:
                        os.kill(process.pid, signal.SIGTERM)
                        log_info(f"[{project_name}] SIGTERM 전송: PID {process.pid}")
                    except (ProcessLookupError, OSError):
                        pass
                ...
                for project_name in running_projects:
                    ...
                    try:
                        ...
                        log_info(f"[{project_name}] WFC 정상 종료됨")
                    except subprocess.TimeoutExpired:
                        log_warn(f"[{project_name}] WFC 30초 타임아웃 — SIGKILL 전송")
                        try:
                            os.killpg(os.getpgid(process.pid), signal.SIGKILL)
                    ...
                    if task_status in ("waiting_for_human_plan_confirm", "needs_replan"):
                        ...
                        self.update_project_state(
                            project_name, "idle", current_task_id=None
                        )
        ...
        while not self._shutdown_requested:
            try:
                ...
                for project_name, state in self._project_states.items():
                    ...
                    # 대기 중인 task에 사용자 응답이 왔는지 확인 → WFC resume
                    if self._check_waiting_task_response(project_name):
                        continue
