# Agent System Architecture Specification v6

> Claude Code CLI 기반 24시간 자동 개발 시스템
> 최종 정리: 2026-04-07
> 이전 버전: `docs_history/004-agent-system-spec-v5.md` (v5)
...
| 문서 | 역할 | 참고 수준 |
|------|------|-----------|
| **`004-agent-system-spec-v6.md` (이 문서)** | **현행 설계 명세. 모든 구현은 이 문서를 기준으로 합니다.** | **기준 문서** |
| `011-handoff-phase-2.1.md` | Phase 2.1 merge_strategy + PR Watcher 핸드오프 | Phase 2.1 상세 |
| `010-handoff-phase-2.0-web-console.md` | Phase 2.0 Web Console 핸드오프 | Phase 2.0 상세 |
| `docs_history/004-agent-system-spec-v5.md` | v5 설계 (아카이브) | 참고 불필요 |
...
- **human interaction:** pending, approve, reject, feedback, complete_pr_review
...
│
├── docs_for_claude/
│   ├── 004-agent-system-spec-v6.md     # v6 설계 (현행 기준 문서) ★
...
| | `scripts/hub_api/protocol.py` | Protocol layer: Request/Response envelope, dispatch, ACTION_REGISTRY (20 actions) |
...
| | `scripts/hub_api/protocol.py` | `complete_pr_review` action 등록 (ACTION_REGISTRY 20개) |
...
| **Pipeline resume** | 실패 지점부터 자동 재개 (TM 연동). 정책 미결정: resume vs 취소+재등록 | 2.2+ |
| **Merge conflict 처리** | git_merge_pr() 에러 시 사용자 noti | 2.2+ |
| **task 순서 변경** | 큐 내 task 우선순위 변경 | 2.2+ |
...
| **1.5** | Chatbot 레이어, 세션 관리, 프로토콜 body (20 actions) | **완료** |
...
|  | └ Web 오류/사용성 개선, 웹 채팅 (async claude -p) | **다음** |
| **2.1** | merge_strategy 3종 + complete_pr_review + PR Watcher + branch config + agent 프롬프트 제한 (197개 테스트) | **완료** |
| **2.2** | 고급 기능: Pipeline resume, user_preferences, GH_TOKEN 환경변수, Merge conflict, task 순서 변경 | 미착수 |
