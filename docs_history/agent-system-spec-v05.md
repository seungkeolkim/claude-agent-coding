# Agent System Architecture Specification v5

> Claude Code CLI 기반 24시간 자동 개발 시스템
> 최종 정리: 2026-04-06
> 이전 버전: `docs_history/003-agent-system-spec-v4.md` (v4)
...
| 문서 | 역할 | 참고 수준 |
|------|------|-----------|
| **`004-agent-system-spec-v5.md` (이 문서)** | **현행 설계 명세. 모든 구현은 이 문서를 기준으로 합니다.** | **기준 문서** |
| `010-handoff-phase-2.0-web-console.md` | Phase 2.0 Web Console 핸드오프 | Phase 2.0 상세 |
| `005-design-history-archive.md` | v1→v2 변경 배경 + Phase 1.0 핸드오프 병합 아카이브 | 설계 배경이 궁금할 때 |
| `docs_history/003-agent-system-spec-v4.md` | v4 설계 (아카이브) | 참고 불필요 |
...
- **git 작업:** branch 생성(base_branch 기준), subtask별 commit+push, Summarizer 후 PR 생성, merge_strategy에 따라 auto_merge/pr_and_continue/require_human 처리
...
**task 큐 블로킹:** `wait_for_prev_task_done=true` (기본값)이면, 이전 task가 `in_progress`, `waiting_for_human_pr_approve`, `waiting_for_human_plan_confirm` 등 미완료 상태일 때 다음 task를 spawn하지 않는다. 프로젝트별로 override 가능.
...
│
├── docs_for_claude/
│   ├── 004-agent-system-spec-v5.md     # v5 설계 (현행 기준 문서) ★
│   ├── 003-agent-system-spec-v4.md     # v4 설계 (아카이브)
...
| | `scripts/hub_api/protocol.py` | ACTION_REGISTRY 확장 (14 → 20개: create_project, resubmit, get_plan, close_project, reopen_project, complete_pr_review) |
...
| **Phase 2.1** | HubAPI: `complete_pr_review()` | waiting_for_human_pr_approve → completed/failed 수동 전이 |
| | WFC: `merge_strategy` enum | require_human / pr_and_continue / auto_merge (auto_merge bool 대체) |
| | WFC: `base_branch` 설정 | feature branch 생성 기준 브랜치 (pr_target_branch와 분리) |
| | TM: PR Watcher 스레드 | 60초 주기 gh pr view로 PR MERGED/CLOSED 자동 감지 |
| | TM: 폴링 주기 변경 | 기본값 5초 → 2초 |
| **테스트** | `tests/` (197개+) | Unit/Integration/E2E + Web DB/Syncer + Lifecycle + PR Review 테스트 |
...
| merge_strategy 분기 | auto_merge (자동 머지), require_human (waiting_for_human_pr_approve), pr_and_continue (즉시 완료) |
...
| 범위 | 내용 | 예정 Phase |
|------|------|-----------|
| **Task lifecycle 정립** | status 전이 규칙 정비, 비정상 전이 방지, waiting_for_human_pr_approve 처리 흐름 | 2.0 (잔여) |
| **Web 오류 수정 + 사용성 개선** | 필터/표시 개선, 액션 버튼 동작 검증, UX 다듬기 | 2.0 (잔여) |
| **웹 채팅** | `scripts/web/web_chatbot.py` — async claude -p 호출, Chat 탭 완성 | 2.0 (잔여) |
| **user_preferences slot** | project_state.json에 사용자 선호 저장. 기존 4계층 내 처리 | 2.1 |
| **GH_TOKEN 환경변수 전환** | 멀티유저 시 gh 토큰을 환경변수로 격리 (현재 시스템 로그인 공유) | 2.1 |
| **Pipeline resume** | 실패 지점부터 자동 재개 (TM 연동). 정책 미결정: resume vs 취소+재등록 | 2.1 |
| ~~SQLite 전환~~ | ~~Phase 2.0에서 하이브리드 방식으로 구현 완료~~ | ~~2.0~~ |
| **Merge conflict 처리** | git_merge_pr() 에러 시 사용자 noti | 2.1 |
| **task 순서 변경** | 큐 내 task 우선순위 변경 | 2.1 |
| **강제 실행 옵션** | wait_for_prev_task_done 무시 force 타입 요청 | 2.1 |
| **메신저 연동** | Slack/Telegram 메시지 수신 → task 생성. Chatbot 경유 | 2.2 |
| **E2E 테스트장비 연동** | e2e_watcher.sh, 크로스 머신 handoff, SSH 복구 | 2.3 |
| **로컬 E2E 테스트** | 브라우저 없는 API E2E를 로컬 실행. mode: local/remote 옵션 | 2.3 |
...
|  | └ Task lifecycle 정립 + Web 오류/사용성 개선 | **다음** |
|  | └ 웹 채팅 (async claude -p) | 예정 |
| **2.1** | 고급 기능: Pipeline resume, user_preferences, GH_TOKEN 환경변수, Merge conflict, task 순서 변경 | 미착수 |
| **2.2** | Messenger (Slack/Telegram) | 미착수 |
| **2.3** | E2E 테스트장비 연동, 로컬 E2E (웹 개발 이후) | 미착수 |

참고: Phase 1.1(파이프라인 자동화)과 1.2(Planner+Subtask Loop)는 Phase 1.0에서 WFC로 통합 구현됨. E2E 테스트장비 연동은 1.3→1.7→2.3으로 재배치 (장비 사정 + 관리 부담으로 웹 이후로 연기).
