...
```
Subtask 1 완료(Reviewer approved) → 커밋
  │  WFC가 final_changes_made / final_intent_report를 subtask JSON에 영속
  ▼
Subtask 2 시작 (run_subtask_pipeline 진입 직전)
  WFC가 subtask JSON에 2개 필드를 주입:
    - prior_changes : 완료된 subtask들의 final_* 수집 (§8.4)
    - plan_position : index/total/strategy_note/siblings(current|completed|upcoming)
  Coder 프롬프트 = 사용자 원문(§8.1의 user_request_raw) + 프로젝트 정보
                  + Task/Subtask/Plan Context JSON (위 두 필드 포함)
```

**사용자 원문 보존 (2026-04-20 추가).** chatbot/Web에서 자연어로 들어온 요청은 title/description으로 재해석될 때 의도가 일부 유실된다. 이를 방지하기 위해 chatbot/Web이 submit 시점에 사용자 입력 원문을 `user_request_raw` 필드로 task JSON에 저장하고, `run_claude_agent.sh`가 모든 agent의 프롬프트 최상단(role 바로 다음)에 "사용자 원문 요청 (최우선 근거)" 섹션으로 노출한다. Planner/Coder/Reviewer 프롬프트는 "title/description/subtask와 원문이 충돌하면 원문 의도를 따른다"를 명시한다. CLI submit 경로에서는 해당 필드가 null이며, 이 경우 title+description이 1차 근거 역할을 한다.
...
```json
{
  ...
  "title": "로그인 기능 구현",
  "description": "...",
  "user_request_raw": "로그인 페이지 만들어줘. 이메일 + 비번으로 ...",
  "submitted_via": "cli",
...
**알림 채널용 plan 요약 (plan_review/replan_review):** WFC가 `request_human_review()`에서 `plan.json`을 읽어 `details.plan_summary`를 함께 주입한다 — `strategy_note`(400자 말줄임) + 각 subtask의 `title`(120자) / `responsibility`(200자), 총 3500자 예산 초과 시 꼬리 항목은 `truncated=True` 마커로 축약. Telegram formatter는 이 요약을 blockquote+bullet으로 렌더링해 Web 없이도 Telegram에서 plan 내용을 검토할 수 있다. `plan_path`가 상대경로인 경우 `project_dir` 기준으로 절대화한 뒤 읽는다.

### 8.3 Plan, Subtask State, E2E Handoff/Result

v2와 동일. 상세는 `docs_history/003-agent-system-spec-v2.md` 섹션 8.2~8.5 참고.

### 8.4 Subtask 런타임 컨텍스트 (재시도 루프 + 전체 맥락)

WFC가 subtask JSON에 주입하는 런타임 필드는 두 범주로 나뉜다.

**(A) 재시도 루프 휘발성 필드** — Coder/Reviewer 호출 직전에 매 attempt마다 갱신:
...
**(B) 전체 맥락 필드 (2026-04-20 추가)** — subtask loop 진입 시점에 1회 주입:
...
```json
{
  "plan_position": {
    "index": 1,
    "total": 3,
    "strategy_note": "사용자 원문의 핵심 의도와 3개 subtask로 나눈 이유 1~2줄 요약",
    "siblings": [
      { "subtask_id": "00042-1", "title": "...", "primary_responsibility": "...", "status": "completed" },
      { "subtask_id": "00042-2", "title": "...", "primary_responsibility": "...", "status": "current" },
      { "subtask_id": "00042-3", "title": "...", "primary_responsibility": "...", "status": "upcoming" }
    ]
  },
  "prior_changes": [
    { "subtask_id": "00042-1", "title": "...", "primary_responsibility": "...",
      "changes_made": [ { "file": "...", "change_type": "created", "summary": "..." } ],
      "intent_report": { "what_changed": "...", "why": "...", "review_focus": [...] } }
  ],
  "final_changes_made": [ ... ],
  "final_intent_report": { ... }
}
...
- `plan_position.siblings[*].status`는 현재 실제 `completed_subtasks`를 기준으로 계산되어, re-plan 이후 완료 목록이 초기화된 상태에서도 정확히 반영된다.
- `prior_changes`의 각 엔트리는 완료 subtask의 `final_changes_made`/`final_intent_report` 필드에서 수집된다. 이 두 필드는 **Reviewer approved 시점**에 `run_subtask_pipeline()`가 해당 subtask JSON에 승격 저장한 값으로, 재시도 중인 임시 값이 아닌 "그 subtask의 최종 결과"다. approved 없이 종료된 subtask는 `completed_subtasks`에 들어가지 않으므로 prior_changes 수집 대상에서도 제외된다.
- 세 주입 지점: `run_pipeline`(신규), `run_pipeline_resume`(중단 후 재개), `run_pipeline_from_subtasks`(re-plan). 모두 `_inject_subtask_overall_context()` 단일 헬퍼를 호출한다.
...
| **Telegram Plan Review UX** | `scripts/workflow_controller.py` | `_build_plan_summary()` + 상수 4종(총 3500자 예산, strategy_note 400자, subtask title 120자/responsibility 200자 말줄임, 예산 초과 시 꼬리 항목 `truncated=True`). `request_human_review()`·재알림 분기에서 `plan_path`를 `project_dir` 기준으로 절대화 후 `details.plan_summary` 주입 |
| | `scripts/telegram/formatter.py` | plan_summary → 전략 노트 blockquote + "Subtasks (N개)" 리스트 렌더. `plan_review_requested`/`replan_review_requested` 이벤트에서만. MarkdownV2 escape 준수, truncated entry는 italic |
| | `scripts/telegram_bridge.py` | "📝 수정" 버튼 → 즉시 reject 대신 force_reply 프롬프트 전송. `data/telegram_pending_modify.json` pending state(TTL 10m, topic 키=`{chat_id}_{thread_id}`). 다음 natural_message를 피드백으로 소비해 `hub_api.reject(message=...)` 호출. 슬래시 명령이 먼저 오면 pending 취소 후 원래 명령 처리 |
| **테스트** | `tests/` (278개) | Unit/Integration/E2E + Web DB/Syncer + Lifecycle + PR Review + merge_pr/close_pr + Web Chat + graceful shutdown/resume + safety limiter + priority queue + Telegram(client/formatter/router/plan_summary/pending_modify) 테스트 |
...
| Telegram plan 상세 표시 | test-web-service task #00005, Telegram Plan Review 알림에 strategy_note + subtask 목록(제목·책임)이 렌더되어 Web 없이 내용 검토 가능. 초기 버그(상대 plan_path로 인해 plan_summary=null 주입)도 `project_dir` 기준 절대화 패치 후 재기동해 정상 수신 확인. "📝 수정" 버튼 → force_reply → 피드백 소비 경로는 유닛 테스트만 통과, live 왕복 검증은 미실시(후속 세션 예정) |
...
|  | └ Plan Review 상세 표시(`details.plan_summary`) + "📝 수정" force_reply로 피드백 입력 → `reject(message=…)` dispatch. 핸드오프: `docs_for_claude/025-handoff-show-plan-in-telegram.md` | 완료 (수정 피드백 live 미검증) |
