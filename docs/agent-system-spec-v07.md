...
- **현재**: `scripts/run_claude_agent.sh:645`에서 `claude --model M -p PROMPT` 호출. `--resume`/`--session-id` 미사용 → 매 호출 cold start. 같은 task의 같은 agent가 attempt 간 컨텍스트를 잃는다(§15.3 TODO 참고).
- **컨텍스트 전달**: 세션이 매번 새로 뜨므로 모든 컨텍스트는 prompt에 명시 주입 + JSON 파일(`subtask-NN.json`의 attempt_history 등)로 전달.
- **codebase CLAUDE.md**: claude는 `cd "$CODEBASE_PATH"` 후 실행되므로 codebase 안에 CLAUDE.md가 있으면 자동 인식. 다만 현재 어느 codebase에도 CLAUDE.md가 없어 효과 없음 (§15.3 TODO).
...
| Summarizer 차단 버그 fix | task 00147에서 마지막 subtask 후 Summarizer가 막히던 `subtask 개수 초과: N+1/N` 재현 후, finalize_task에서 current_subtask 클리어 + safety 중복 집계 방어 적용으로 해소 확인 |

### 15.3 미구현 (TODO)

| 범위 | 내용 | 예정 Phase |
|------|------|-----------|
| **claude -p 세션 재사용** | 현재 매 agent 호출이 cold start (`run_claude_agent.sh:645`에 `--resume`/`--session-id` 없음). task+agent 단위로 세션 ID 부여하여 attempt 간 컨텍스트 유지 검토. 토큰 누적/잘못된 결정 고착 위험 관리 필요 | 2.5 |
| **codebase CLAUDE.md 자동 생성/유지** | claude는 `cd "$CODEBASE_PATH"` 후 실행되어 codebase 안의 CLAUDE.md를 자동 인식하지만 현재 부재. init-project 시 템플릿 생성 + 매 PR merge 후 Summarizer가 갱신 | 2.5 |
| **projects/{name}/PROJECT_NOTES.md** | "이 프로젝트는 X 패턴" 같은 누적 메모를 매 agent prompt에 inject. 가벼운 시작점 | 2.5 |
...
| **Summarizer 산발적 실패** | task 00147에서 Summarizer 호출이 실패해 fallback PR 메시지 사용. 재현/원인 점검 필요 (`projects/test-project/logs/00147/00147_99_08-summarizer.log`) | 미정 |
...
| **2.5** | claude 세션 재사용, codebase CLAUDE.md 자동화, PROJECT_NOTES.md 누적 메모 (§15.3 참고) | 미착수 |
