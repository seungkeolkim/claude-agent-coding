...
| **replan 승인 단계 미집행 버그** | Planner가 replan을 만들어낸 뒤 human_review_policy.review_replan=true여도 사용자 검토 없이 바로 다음 단계로 넘어가는 것으로 관찰됨. WFC의 replan 분기(`workflow_controller.py` 내 replan 후 `waiting_for_human_plan_confirm` 진입 경로)와 `review_replan` 플래그 검사 누락/오용 여부 점검 필요. 재현 케이스 확보 후 수정 | 미정 |
