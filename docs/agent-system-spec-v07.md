...
| `docs_for_claude/014-handoff-priority-queue.md` | Priority Queue (Phase 2.2) 핸드오프 — 미구현 | **다음 작업** |
...
| 범위 | 내용 | 예정 Phase |
|------|------|-----------|
| **Priority Queue** | `.ready` sentinel → `task_queue_{critical,urgent,default}.json` 기반 우선순위 큐 전환. `fcntl.flock` 동시성 제어 (`docs_for_claude/014-handoff-priority-queue.md`) | 2.2 |
...
| **2.2** | Priority Queue (`.ready` sentinel → `task_queue_*.json` 전환, `fcntl.flock` 동시성) — `014` 핸드오프 | **다음** |
