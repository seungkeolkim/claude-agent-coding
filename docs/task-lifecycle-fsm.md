...
stateDiagram-v2
    ...
    submitted --> waiting_for_human : Planner 성공\n(review_plan=true)
    submitted --> failed : Planner 실패\n/ subtask 0개
    submitted --> cancelled : 사용자 cancel

    waiting_for_human --> planned : approve\n/ timeout 자동승인
    waiting_for_human --> needs_replan : reject (modify)
    waiting_for_human --> cancelled : cancel

    needs_replan --> waiting_for_human : Replanner 성공\n(review_replan=true)
    ...
    in_progress --> pending_review : 모든 subtask 완료\n+ PR auto_merge=false
    in_progress --> failed : Agent 실행 실패\n/ git push 실패\n/ PR 생성 실패
    in_progress --> cancelled : 사용자 cancel

    pending_review --> completed : PR 수동 머지 (외부)
    pending_review --> failed : PR 거부/닫힘 (외부)
...
| waiting_for_human | Planner 성공 + review_plan=true | `workflow_controller.py` request_human_review() |
...
| 대상 | 트리거 | 위치 |
|------|--------|------|
| waiting_for_human | Replanner 성공 + review_replan=true | `workflow_controller.py` run_pipeline() |
...
| pending_review | 모든 subtask 완료 + auto_merge=false | `workflow_controller.py` finalize_task() |
...
| waiting_for_human | 프로젝트 레벨: human review 대기 |
...
```python
incomplete_statuses = {"in_progress", "planned", "running", "waiting_for_human", "needs_replan"}
```
