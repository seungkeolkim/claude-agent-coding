...
TM → WFC 명령 전달:
  ...
  예) pause.command, resume.command, cancel-TASK-042.command

WFC → TM 보고:
  projects/{name}/project_state.json 업데이트 (TM이 읽기)

WFC ← task 수신:
  projects/{name}/tasks/TASK-{id}.ready sentinel 감지
```
...
- **상세:**
  - 작업 큐 관리 및 우선순위 결정
  - `projects/{name}/tasks/TASK-{id}.json` 생성
  - 첨부 이미지 다운로드 → `projects/{name}/attachments/TASK-{id}/`에 저장
...
```
 1. 사용자: CLI submit (또는 메신저)
    → Task Manager: 프로젝트 식별 → projects/{name}/tasks/TASK-{id}.json 생성
...
```bash
# 1. tmp 파일에 쓰기
echo '{"task_id": "TASK-042", ...}' > projects/my-app/tasks/TASK-042.json.tmp
# 2. rename (atomic)
mv projects/my-app/tasks/TASK-042.json.tmp projects/my-app/tasks/TASK-042.json
# 3. sentinel 생성 (읽기 트리거)
touch projects/my-app/tasks/TASK-042.ready
```
...
touch projects/my-app/commands/cancel-TASK-042.command
```
...
SCP로 결과 업로드:
```powershell
scp ./e2e-result.json server2:agent-hub/projects/my-app/handoffs/TASK-042-2-e2e-result.json
scp -r ./screenshots/ server2:agent-hub/projects/my-app/logs/TASK-042/screenshots/
ssh server2 "touch agent-hub/projects/my-app/handoffs/TASK-042-2-e2e-result.ready"
```
...
### 8.1 Task (projects/{name}/tasks/TASK-042.json)
...
### 8.2 Plan (projects/{name}/tasks/TASK-042-plan.json)
...
### 8.3 Subtask State (projects/{name}/tasks/TASK-042-1.json)
...
### 8.4 E2E Handoff (projects/{name}/handoffs/TASK-042-2-e2e.json)
...
### 8.5 E2E Result (projects/{name}/handoffs/TASK-042-2-e2e-result.json)
...
├── .env                                # credential (gitignored)
├── .env.template                       # credential 템플릿
...
│   │   └── (pause.command, cancel-TASK-042.command 등)
│   │
│   ├── attachments/
│   │   └── TASK-042/
│   │       ├── ui_mockup.png
...
# Plan 승인
./run_agent.sh approve TASK-042 --project test-web-service

# Plan 수정 요청
./run_agent.sh reject TASK-042 --project test-web-service \
  --message "소셜 로그인도 추가해줘" \
  --attach social_login_flow.png

# 진행 중 task에 피드백
./run_agent.sh feedback TASK-042 --project test-web-service \
...
# Task 일시정지 / 재개 / 취소
./run_agent.sh pause TASK-042 --project test-web-service
./run_agent.sh resume TASK-042 --project test-web-service
./run_agent.sh cancel TASK-042 --project test-web-service
...
- `run_agent.sh run <agent_type> --project <name> --task TASK-001` 으로 agent 하나를 직접 실행
...
`config.yaml.template` → `create_config_and_env.sh` → `config.yaml` (gitignored). 민감 정보(경로, credential)가 git에 올라가는 것을 방지.
...
| `create_config_and_env.sh` | 존재. v2 구조에 맞게 수정 필요. |
