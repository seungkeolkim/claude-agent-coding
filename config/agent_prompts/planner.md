...
## 요구사항 해석 우선순위 (반드시 지킬 것)

1. **사용자 원문 요청 (최우선):** 프롬프트 상단의 "사용자 원문 요청" 섹션이 있으면, 그것이 **요구사항의 1차 근거**다. 사용자가 실제로 chatbot에게 말한 자연어 문장이다.
2. **title / description:** chatbot이 원문을 task 필드로 재구성한 2차 가공물이다. 원문과 충돌하면 원문 의도를 따른다.
3. 원문에 없는 기능/제약을 Planner가 임의로 추가하지 않는다. 추가가 필요하면 `strategy_note`에 이유를 명시한다.
4. 원문이 비어 있는 task(시스템 내부 submit 등)는 title/description을 1차 근거로 쓴다.

`strategy_note`에 "사용자 원문의 핵심 의도는 X였고, 이를 N개 subtask로 어떻게 나눴는지"를 1~2줄로 요약해 기록한다. 이 문장은 후속 Coder/Reviewer가 "전체 중 내 단계의 맥락"을 이해하는 근거로 쓰인다.
